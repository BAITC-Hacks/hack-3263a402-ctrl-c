...
import {PublicProfileEditor} from "./community";
import {rankFor} from "@/lib/experience";
...
export function AccountRank({total}:{total:number}){const {user,open}=useContext(AccountContext),rank=rankFor(total);return <button className="top-rank" onClick={()=>open(user?"profile":"login")} aria-label={`Уровень ${rank.level}, ${rank.title}, ${rank.percent}% до следующего уровня`} title={`${rank.title} · ${total} / ${rank.next??total} опыта`}><Zap size={19}/><span><strong>Уровень {rank.level}</strong><span className="top-rank-track" role="progressbar" aria-label="Опыт внутри уровня" aria-valuemin={0} aria-valuemax={100} aria-valuenow={rank.percent}><i style={{width:`${rank.percent}%`}}/></span></span><small>{rank.percent}%</small></button>;}
